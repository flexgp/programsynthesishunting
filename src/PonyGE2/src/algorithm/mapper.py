...
def derivation_dic_from_tree(tree):
    deriv_dic = {}
    node_list = [tree]
    while len(node_list) > 0:
        ...
        if len(current_node.children) == 0:
            continue
        if current_node.root in deriv_dic:
            deriv_dic[current_node.root] += 1
        else:
            deriv_dic[current_node.root] = 1
...
def map_ind_from_genome(genome):
    ...
    while (wraps < max_wraps) and unexpanded_symbols:
        ...
        if current_symbol["type"] != "NT":
            ...
            #Update symbol/derivation dic
            if current_symbol["symbol"] not in symbol_dic:
                symbol_dic[current_symbol["symbol"]] = 1
            else:
                symbol_dic[current_symbol["symbol"]] += 1
    ...
    if len(unexpanded_symbols) > 0:
        ...
        # solution.
        return None, genome, None, nodes, True, max_depth, used_input, symbol_dic

    return output, genome, None, nodes, False, max_depth, used_input, symbol_dic
